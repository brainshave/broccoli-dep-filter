...
function filter (config) {
  ...
  function process_file (process, root, path) {
    ...
    return Q(process(param)).then(function (result) {
      ...
      var deps = _.uniq([input_path].concat(drain(), result.deps));

      console.log(tree_name, deps);
      ...
      fs.writeFileSync(output_path, result.content, fs_options);
